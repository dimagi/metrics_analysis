...
HostStats = namedtuple('HostStats', 'name, memory, swap, cpu_logical_processors, disk, root_disk, boot_disk')
...
def get_host_stats(env_name):
    ...
    for host in infra_content['rows']:
        ...
        root_disk = kb_to_gb('{}kB'.format( sum(int(drive['kb_size']) for drive in specs['filesystem'] 
                    if drive['mounted_on'] == '/' )))
        boot_disk = kb_to_gb('{}kB'.format( sum(int(drive['kb_size']) for drive in specs['filesystem'] 
                    if drive['mounted_on'] == '/boot' )))        
...
def get_host_usage_stats(env_name):
    ...
    def add_highest_boot_disk_in_last_week():
        for host in host_stats_by_host:
            usage_stats_by_host[host]['boot_disk'] = 0
        for boot_drive in ['/dev/vda1']:
            query = 'sum:system.disk.used{environment:%s,device:%s}by{host}' % (env_name, boot_drive)
            disk_stats = get_pointlist_by_host(api.Metric.query(start=now - 604800, end=now, query=query))
            for host, pointlist in disk_stats.items():
                new_value = max(value for _, value in pointlist) / 1024 ** 3
                usage_stats_by_host[host]['boot_disk'] = max(usage_stats_by_host[host]['boot_disk'], new_value)   
    ...
    add_highest_boot_disk_in_last_week()
...
def print_hosts(env_name):
    print('{}\t{}\t{}\t{}\t{}\t{}\t{}'.format('Name', 'Memory (GB)', 'Swap (GB)', 'Logical Processors', 'Disk (GB)', 'ROOT DISK(GB)', 'BOOT DISK(GB)'))
    for host_stats in get_host_stats(env_name):
        print('{name}\t{memory}\t{swap}\t{cpu_logical_processors}\t{disk}\t{root_disk}\t{boot_disk}'.format(**host_stats._asdict()))


def print_host_usage(env_name):
    print('{}\t{}\t{}\t{}\t{}\t{}\t{}'.format('Name', 'Memory (GB)', 'Swap (GB)', 'Logical Processors', 'Disk (GB)','ROOT DISK(GB)', 'BOOT DISK(GB)'))
    for host_stats in get_host_usage_stats(env_name):
        print('{name}\t{memory:.1f}\t{swap:.1f}\t{cpu_logical_processors:.1f}\t{disk:.1f}\t{root_disk:.1f}\t{boot_disk:.2f}'.format(**host_stats._asdict()))
